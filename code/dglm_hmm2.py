...
class dGLM_HMM2():
    ...
    def simulate_data(self, trueW, trueP, truepi, sessInd, save=False, title='sim'):
        ...
        if (save==True):
            np.save(f'../data_M2/{title}x', x)
            np.save(f'../data_M2/{title}y', y)
            np.save(f'../data_M2/{title}z', z)
    ...
    def fit(self, x, y, present, initP, initpi, initW, sigma, alpha, globalP, sessInd=None, maxIter=250, tol=1e-3, L2penaltyW=1, fit_init_states=False):
        '''
            ...
            for s in range(0,sess):
                ...
                for i in range(0, self.k):
                    for j in range(0, self.k):
                        p[sessInd[s]:sessInd[s+1],i,j] = (zetaSess[:,i,j].sum() + alpha * globalP[i,j])/(zetaSess[:,i,:].sum() + alpha) # closed form update
